...
static bool NandFlash_RequestWrite(Ptr<MB> &fwb, u16 devID, bool updateCRC) { return true; }
...
static Ptr<REQ> CreateTrmReq03(u16 tryCount)
{
	...
	rq->rsp = AllocMemBuffer(sizeof(RspTrm03)+2);
	if (!rq->rsp.Valid()) { rq.Free(); return rq; };
...
static u32 InitRspMan_20(u16 rw, __packed u16 *data)
{
	...
	*(data++) = temp2;//min;		 			//14. Минимальное значение температуры в приёмниках (short)(0.1гр)
	*(data++) = temp2;//max;		 			//15. Максимальное значение температуры в приёмниках (short)(0.1гр)
...
static void StartTrmFire()
{
	...
	PIO_TRMRTS->SET(TRMRTS);
	PIO_RCVRTS->SET(RCVRTS);
...
static void StopTrmFire()
{
	...
	PIO_TRMRTS->CLR(TRMRTS);
	PIO_RCVRTS->CLR(RCVRTS);
...
Ptr<MB> CreateTestRspRcv02()
{
	...
	rq = AllocMemBuffer(sizeof(RspRcv02));
...
static void UpdateTestFlashWrite()
{
	...
	if (mv.disableFireNoVibration != 0 && mv.levelNoVibration == 0xFFFF)
	{
...
static const u32 trmFlashPages[] = { 0
